...
class GaussianData(Dataset):
    def __init__(self, midpoints, clusterLabel, std):
        super().__init__()        
    ...
    def showData(self):
        ...
        if self.num_dimensions == 2:
            for dist in range(self.num_clusters):
                ...
                input, output = self.__getitem__(index)
                plt.scatter(input[:, 0], input[:, 1], marker='o', label="Data Points"+str(dist+1), colorizer=output)
